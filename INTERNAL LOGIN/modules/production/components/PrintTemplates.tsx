...
import { Bill, Client, FilmType, BillItem, LedgerPayment, FilmItem } from '../../../types';
import { formatDate, amountToWords } from '../utils';
import { today } from '../utils';

export const PrintQuotationModal: React.FC<{ bill: Bill; client: Client | undefined; filmTypes: FilmType[]; onClose: () => void }> = ({ bill, client, filmTypes, onClose }) => {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            window.print();
            setTimeout(() => { document.title = "Aarya Attendance App"; }, 1000);
        }, 500);
    ...
    const subTotal = bill.subTotal || bill.totalAmount; // Fallback for old records
    const freight = bill.freight || 0;
    const discount = bill.discount || 0;
    const grandTotal = bill.totalAmount;

    return (
        <div className="printable-content-wrapper fixed inset-0 bg-white z-[200] overflow-y-auto">
            <div className="max-w-4xl mx-auto p-8 h-full flex flex-col relative print-content">
                <button onClick={onClose} className="absolute top-4 right-4 print:hidden px-4 py-2 bg-gray-200 rounded-md">Close</button>
...
export const PrintChecklistModal: React.FC<{ bill: Bill; client: Client | undefined; filmTypes: FilmType[]; inventory: FilmItem[]; onClose: () => void }> = ({ bill, client, filmTypes, inventory, onClose }) => {
    // Generate dynamic filename
    const getFilename = () => {
        const clientName = client?.name?.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'client';
        const dateStr = bill.date?.replace(/-/g, '') || 'date';
    ...
    const groupedItems = useMemo(() => {
        if (!bill.items) return [];
        const filmTypeMap = new Map(filmTypes.map(ft => [ft.id, ft.name]));
        // Create lookup map for inventory items to get date and gross weight
        ...
        const grouped = bill.items.reduce((acc, item) => {
            ...
            // Lookup extra data from inventory map using item.itemId
        ...
        return Object.values(grouped).map((g) => {
            // Sort items numerically by box number
            g.boxes.sort((a, b) => {
    ...
    // Counter for global serial number
    let globalSlNo = 0;
        ...
        <div className="printable-content-wrapper fixed inset-0 bg-white z-[200] overflow-y-auto">
             <div className="max-w-4xl mx-auto p-8 h-full flex flex-col relative print-content">
                ...
                <div className="mb-8">
                        ...
                        <tbody>
                            {groupedItems.map((group) => (
                                <React.Fragment key={group.filmTypeName}>
                                    <tr className="bg-gray-100 break-inside-avoid print:bg-gray-100">
                                    ...
                                    {group.boxes.map((box) => {
                                       globalSlNo++;
                                       return (
                                           <tr key={box.itemId} className="border-b border-gray-100 last:border-0 break-inside-avoid">
                 ...
                 <div className="print-footer mt-auto pt-8 border-t border-gray-200 flex justify-between break-inside-avoid">
...
export const PrintLedgerModal: React.FC<{ 
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            document.title = `Statement_${client.name}_${dateRange.start}_to_${dateRange.end}`;
            window.print();
            setTimeout(() => { document.title = "Aarya Attendance App"; }, 1000);
        }, 500);
        return () => clearTimeout(timer);
    ...
    return (
        <div className="printable-content-wrapper fixed inset-0 bg-white z-[200] overflow-y-auto">
            <div className="max-w-5xl mx-auto p-8 h-full flex flex-col relative print-content">
                <button onClick={onClose} className="absolute top-4 right-4 print:hidden px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300">Close</button>
